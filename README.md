...
│   ├── entity/                   # JPA 实体
│   ├── repository/               # 数据访问层
│   ├── security/                 # JWT + AgentKey 认证过滤器
│   ├── engine/                   # 规则引擎（核心亮点）
│   ├── websocket/                # WebSocket 实时推送
│   ├── scheduler/                # 定时任务（心跳检测、统计聚合）
│   ├── consumer/                 # Redis Stream 消费者
│   └── resources/
│       ├── db/migration/         # Flyway 迁移脚本 (13张表)
│       └── rules/                # 内置 YAML 规则文件
│
├── secguard-agent/               # 轻量级 Agent (端口 8901)
│   ├── collector/log/            # 日志采集（文件尾随、轮转检测）
│   ├── collector/fim/            # FIM 文件完整性监控
│   ├── collector/inventory/      # 主机资产采集
│   ├── sender/                   # 事件上报（HTTP + 重试）
...
curl -X POST http://localhost:8900/api/agents/register \
  -H "Content-Type: application/json" \
  -d '{"name":"test-agent","ip":"192.168.1.100","os":"linux"}'
```
