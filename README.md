...
| 层级 | 技术 |
|------|------|
| **后端** | Java 25 + Spring Boot 3.3.5 + Spring Security 6 + Spring Data JPA |
| **数据库** | MySQL 5.7+ + Flyway 数据库迁移 |
| **缓存** | Redis 5.0+ (已配置，预留扩展) |
...
```bash
# 启动 MySQL + Redis + Server（Agent 按需启用）
docker-compose up -d
...
环境变量覆盖（`.env` 文件或 `export`）：

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `MYSQL_ROOT_PASSWORD` | `SecGuard@2026` | MySQL root 密码 |
| `JWT_SECRET` | 内置默认值 | JWT 签名密钥（生产环境必须更换） |
| `SECGUARD_AGENT_SERVER_URL` | `http://server:8900` | Agent 连接 Server 地址 |
